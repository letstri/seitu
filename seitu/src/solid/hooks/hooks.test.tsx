...
function TestComponentWithSelector(props: {
...
describe('useSubscription with selector', () => {
  it('should update when selector value changes', async () => {
    ...
    const { getByTestId } = render(() => (
      <TestComponentWithSelector storage={storage} />
    ))
    expect(getByTestId('subscription-value')).toHaveTextContent('0')
...
describe('useSubscription with custom isEqual', () => {
  it('should use custom comparator instead of deepEqual', async () => {
    ...
    await waitFor(() =>
      expect(getByTestId('subscription-value')).toHaveTextContent('changed')
    )
    expect(runs).toBe(2)
  })
...
describe('useSubscription with reactive source', () => {
  it('should pick up a new subscription when the accessor returns a new source', async () => {
    ...
    const [current, setCurrent] =
      createSignal<WebStorageValue<number>>(storageA)
    ...
    setCurrent(storageB)
    await waitFor(() =>
      expect(getByTestId('subscription-value')).toHaveTextContent('2')
    )

    storageB.set(20)
    await waitFor(() =>
      expect(getByTestId('subscription-value')).toHaveTextContent('20')
    )
  })
...
describe('createMediaQuery', () => {
  it('should return the matches value for light and dark', async () => {
    ...
    let listener: ((event: Event) => void) | null = null as
      | ((event: Event) => void)
      | null
