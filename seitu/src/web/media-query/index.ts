...
type RangeFeature =
  ...
  | 'color-index'
  | 'monochrome'
type RangeValue<F extends string> = F extends 'aspect-ratio'
  ? Ratio
  : F extends 'resolution'
    ? CSSResolution
    : F extends 'color' | 'color-index' | 'monochrome'
      ? `${number}`
      : CSSUnit
type RangeOp = '<' | '<=' | '>' | '>=' | '='
type RangeLt = '<' | '<='
/** Level 4 range syntax, e.g. `(width >= 768px)`. */
type MQRangeSyntax = {
  [F in RangeFeature]:
    | `(${F} ${RangeOp} ${RangeValue<F>})`
    | `(${RangeValue<F>} ${RangeOp} ${F})`
    | `(${RangeValue<F>} ${RangeLt} ${F} ${RangeLt} ${RangeValue<F>})`
}[RangeFeature]
...
type IsValid<T extends string> = string extends T
  ? true
  : T extends `${infer L}, ${infer R}`
    ? IsValid<L> extends true
      ? IsValid<R>
      : false
    : T extends `${infer L} and ${infer R}`
      ? IsValid<L> extends true
        ? IsValid<R>
        : false
      : T extends `not ${infer Rest}`
        ? IsValid<Rest>
        : T extends MediaQueryType
          ? true
          : false
...
type SuggestFeature<F extends string, V extends string> =
  ParseNum<V> extends infer N extends number
    ? `(${F}: ${N}${CSSUnitSuffix})`
    : [ValuesOf<F>] extends [never]
      ? MediaQueryType
      : `(${F}: ${ValuesOf<F>})`

type Suggest<T extends string> = string extends T
  ? Suggestion
  : T extends `${infer L}, ${infer R}`
    ? IsValid<L> extends true
      ? `${L}, ${Suggest<R>}`
    ...
    : T extends `${infer L} and ${infer R}`
      ...
      : T extends `not ${infer Rest}`
        ...
        : T extends `(${infer F}: ${infer Rest}`
          ? SuggestFeature<F, Rest extends `${infer V})` ? V : Rest>
          : MediaQueryType

export interface MediaQueryOptions<T extends string> {
  /**
   * Media query, e.g. `(min-width: 768px)`. Literals are type-checked; widen
   * to `string` to skip.
   */
 ...
 * ```
 */
export function createMediaQuery<T extends string>(
  options: MediaQueryOptions<T>
): MediaQuery {
  const match =
    typeof window === 'undefined' ? null : window.matchMedia(options.query)
  ...
  const get = (): boolean => match?.matches ?? defaultMatches

  return createReadableSubscription(
    get,
    subscribe,
    notify,
    () => defaultMatches
