...
import { createWebStorage } from './index'
...
describe('createWebStorage', () => {
  ...
  describe('set', () => {
    it('stores values in localStorage and notifies subscribers on set and on storage event', () => {
      ...
      // One notification per set() call, not one per key.
  ...
  describe('clear', () => {
    ...
    it('clears keys with keyTransform applied', () => {
      ...
      expect(window.localStorage.getItem('prefix-name')).toBe(
        JSON.stringify('bob')
      )
...
describe('createWebStorage synthetic storage events', () => {
  it('dispatches native-shaped events and skips unchanged writes', () => {
    const storage = createWebStorage({
      type: 'localStorage',
      schemas: { count: z.number() },
      defaultValues: { count: 0 },
    ...
    expect(events[0].storageArea).toBe(window.localStorage)
    ...
    expect(events[1].newValue).toBeNull()

    window.removeEventListener('storage', listener)
  })

  it('exposes defaultValues as the server snapshot', () => {
    const storage = createWebStorage({
      type: 'localStorage',
      schemas: { count: z.number() },
    ...
    storage.set({ count: 1 })
    expect(storage.getServer()).toEqual({ count: 7 })
  })

  it('notifies when the whole storage is cleared externally', () => {
    const storage = createWebStorage({
      type: 'localStorage',
      schemas: { count: z.number() },
      defaultValues: { count: 0 },
