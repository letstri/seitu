...
import { createScrollState } from './index'
...
function setScrollPosition(
  el: Element,
  props: { scrollTop?: number; scrollLeft?: number }
) {
...
describe('createScrollState', () => {
  describe('get', () => {
    ...
    it('supports per-side thresholds', () => {
      ...
      const scroll = createScrollState({
        element: el,
        threshold: { top: 20, bottom: 5, left: 10, right: 30 },
      })
      const state = scroll.get()
  ...
  describe('element getter', () => {
    ...
    it('subscribe calls callback with inactive when getter returns null and immediate is set', () => {
      const scroll = createScrollState({ element: () => null })
      const callback = vi.fn()

      scroll.subscribe(callback, { immediate: true })
  ...
  describe('subscribe', () => {
    ...
    it('calls callback once with default state when element is null and immediate is set', () => {
      const scroll = createScrollState({ element: null })
      const callback = vi.fn()

      const unsubscribe = scroll.subscribe(callback, { immediate: true })
...
describe('createScrollState resize and null element', () => {
  it('notifies when the element resizes (ResizeObserver)', () => {
  ...
  it('still delivers manual notifications when the element is null', () => {
    const scroll = createScrollState({ element: null })
    const callback = vi.fn()
  ...
  it('exposes the inactive state as the server snapshot', () => {
    const scroll = createScrollState({
      element: createMockElement({ scrollHeight: 500, clientHeight: 200 }),
    })
