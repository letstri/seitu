...
import { createComputed } from './index'
...
describe('createComputed', () => {
  ...
  describe('multiple sources', () => {
    ...
    it('works with mixed source types', () => {
      ...
      const label = createComputed([name, age], ([n, a]) => `${n} is ${a}`)
...
describe('createComputed memoization', () => {
  it('does not re-run transform while source values are unchanged', () => {
    const store = createStore({ a: 1 })
    const transform = vi.fn((s: { a: number }) => ({ double: s.a * 2 }))
    const computed = createComputed(store, transform)
    ...
    store.set({ a: 2 })
    expect(computed.get()).toEqual({ double: 4 })
    expect(transform).toHaveBeenCalledTimes(2)
  })

  it('memoizes across multiple sources', () => {
    const a = createStore(1)
    const b = createStore(2)
    const transform = vi.fn(([x, y]: [number, number]) => x + y)
    const sum = createComputed([a, b], transform)
  ...
  it('derives a server snapshot when every source provides one', () => {
    const withServer = {
      ...createStore(5),
      getServer: () => 1,
    }
    const computed = createComputed(withServer, (v) => v * 10)
    expect(computed.getServer?.()).toBe(10)
