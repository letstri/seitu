...
import { createThrottledFn } from './index'
...
describe('createThrottledFn', () => {
  ...
  it('supports async return values', async () => {
    const throttled = createThrottledFn(
...
describe('createThrottledFn cancel/flush/pending', () => {
  it('runs the trailing call on flush and reports pending', () => {
    const fn = vi.fn((x: number) => x)
    const throttled = createThrottledFn(fn, 100)
  ...
  it('cancel drops the trailing call and reopens the window', () => {
    const fn = vi.fn((x: number) => x)
    const throttled = createThrottledFn(fn, 100)
