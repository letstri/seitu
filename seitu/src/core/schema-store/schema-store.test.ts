...
import { createSchemaStore } from './index'
...
describe('createSchemaStore memoization', () => {
  it('validates once per state change and returns a stable reference', () => {
    ...
    const store = createSchemaStore({
      schema,
      defaultValue: { count: 0 } as { count: number },
    })
